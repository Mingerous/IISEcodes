...
def Get_Xtk(tao, Ctao, Dtao, Gxdelay):
        ...
        for t in Yperiods:
            ...
            for k in Ytypes:
                ...
                if k >= T - t:
                    ...
                    model.addConstr(My[n, t, k] >= Y[t, k] - BigM * B3[n, t, k],
                                    name=str(n) + str(t) + str(k))
                    model.addConstr(My[n, t, k] >= demand[n, t, k] - BigM * B4[n, t, k],
                                    name=str(n) + str(t) + str(k))
            ...
            # 违背约束频次
            model.addConstr(Y.sum(t, tuplelist(range(0, min(K, T - t)))) <=
                            demand.sum(n, t, tuplelist(range(0, min(K, T - t)))) + BigM * Z[n], name="d")
    ...
    # 目标函数
    rt = gp.quicksum(Price[tao, k] * Mx[k] for k in Xtypes)
...
def Rolling(minute):
    ...
    # for n in range(0, 1):
        file = open(
            "Files/RollingHorizon/" + P + "/N" + str(Num_of_SAA_samples) + "PAI" + str(
    ...
    f = open("Files/RollingHorizon/" + P + "/N" + str(Num_of_SAA_samples) + "PAI" + str(len(PAI.index)) + "C" + str(
        C0) + "h" + str(h) + "beta" + str(beta) + ".txt", "a")
