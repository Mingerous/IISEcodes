...
def Get_Xtk(tao, Ctao, Dtao, Gxdelay):
    ...
    B5 = model.addVars(Xtypes, vtype=GRB.BINARY, name='B5')
    B6 = model.addVars(Xtypes, vtype=GRB.BINARY, name='B6')
    F = model.addVars(Xtypes, vtype=GRB.INTEGER, name='F')

    # Xtk约束
    model.addConstr(X.sum() <= Ctao, name='b')
    for k in Xtypes:
        model.addConstr(X[k] >= 0, name='c')
    F0 = {}
    for k in Xtypes:
        if k == 0:
            F0[k] = Dtao[k]
        else:
            F0[k] = Dtao[k] + C0 * (gp.quicksum(X[s] for s in range(0, k)) - sum(Dtao[s1] for s1 in range(0, k)))
        model.addConstr(F[k] >= F0[k], name='c2')
        model.addConstr(F[k] >= 0, name='c2')
        model.addConstr(F[k] <= F0[k] + 100 * BigM * B5[k], name='c2')
        model.addConstr(F[k] <= 0 + 100 * BigM * B6[k], name='c2')
        model.addConstr(B5[k] + B6[k] == 1, name='c2')
        model.addConstr(X[k] <= F[k], name='c')
    # tao时刻匹配
    for k in Xtypes:
...
def Rolling(minute):
    ...
    for n in range(0, Num_of_Rolling_days):
        file = open(
            "Files/F" + str(minute) + "ValidInequalities/" + P + "/fairconstraintN" + str(Num_of_SAA_samples) + "PAI" + str(
        ...
        for tao in range(0, Num_of_decisions):
            ...
            unfair1 = 0
            unfair2 = 0
            # file.write('X' + str(tao) + ':')
            for k in range(0, min(Num_of_types, Num_of_periods - int(tao * minute / 30))):
                Xtk[tao, k] = int(X[k])
                # file.write(str(Xtk[tao, k]) + ',')
            # file.write('\n')
            for k in range(0, min(Num_of_types, Num_of_periods - int(tao * minute / 30))):
                if Xtk[tao, k] < Dtk[tao, k] and sum(
                        Xtk[tao, k1] for k1 in range(k + 1, Num_of_periods - int(tao * minute / 30))) >= 1:
                    unfair1 += 1
                if Dtk[tao, k] > 0 and Xtk[tao, k] == 0 and sum(
                        Xtk[tao, k1] for k1 in range(k + 1, Num_of_periods - int(tao * minute / 30))) >= 1:
                    unfair2 += 1
            # file.write(str(tao) + ':' + str(round(unfair1 / (Num_of_periods - int(tao * minute / 30)), 2)) + ',' + str(
            #     round(unfair2 / (Num_of_periods - int(tao * minute / 30)), 2)) + '\n')
    ...
    f = open("Files/F" + str(minute) + "ValidInequalities/" + P + "/N" + str(Num_of_SAA_samples) + "PAI" + str(
            len(PAI.index)) + "C" + str(C0) + "h" + str(h) + "beta" + str(beta) + ".txt", "a")
